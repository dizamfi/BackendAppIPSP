...
const { generarTags, generarTagsTA, arregloTagsPC11Antenas } = require('../../../helpers/california/arregloTagsAireacion');
let interval = null; 
let dataCache = {
    PC11: [],
    PC11TA: [],
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PC11Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(5)] = ex.Value;     
                    });
                    dataCache.PC11Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPC11 = (ws, message) => {
    if (message.toString() === 'subscribePC11'){
    ...
    } else if (message.toString() === 'unsubscribePC11'){
...
module.exports = { handleSubscriptionPC11 }
