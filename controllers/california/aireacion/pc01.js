...
const { generarTags, generarTagsTA, arregloTagsPC01Antenas } = require('../../../helpers/california/arregloTagsAireacion');
let interval = null; 
let dataCache = {
    PC01: [],
    PC01TA: [],
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PC01Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(5)] = ex.Value;     
                    });
                    dataCache.PC01Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPC01 = (ws, message) => {
    if (message.toString() === 'subscribePC01'){
    ...
    } else if (message.toString() === 'unsubscribePC01'){
...
module.exports = { handleSubscriptionPC01 }
