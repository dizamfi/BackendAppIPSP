...
const { generarTags, generarTagsTA, generarTagsTCP, arregloTagsPC18Antenas } = require('../../../helpers/california/arregloTagsAireacion');
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PC18Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(5)] = ex.Value;     
                    });
                    dataCache.PC18Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPC18 = (ws, message) => {
    if (message.toString() === 'subscribePC18'){
    ...
    } else if (message.toString() === 'unsubscribePC18'){
...
module.exports = { handleSubscriptionPC18 }
