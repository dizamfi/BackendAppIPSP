...
const { generarTags, generarTagsTA, generarTagsTCP, arregloTagsPC14Antenas } = require('../../../helpers/california/arregloTagsAireacion');
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PC14Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(5)] = ex.Value;     
                    });
                    dataCache.PC14Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPC14 = (ws, message) => {
    if (message.toString() === 'subscribePC14'){
    ...
    } else if (message.toString() === 'unsubscribePC14'){
...
module.exports = { handleSubscriptionPC14 }
