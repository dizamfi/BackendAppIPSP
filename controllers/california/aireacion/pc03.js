...
const { generarTags, generarTagsTA, generarTagsTCP, arregloTagsPC03Antenas } = require('../../../helpers/california/arregloTagsAireacion');
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PC03Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(5)] = ex.Value;     
                    });
                    dataCache.PC03Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPC03 = (ws, message) => {
    if (message.toString() === 'subscribePC03'){
    ...
    } else if (message.toString() === 'unsubscribePC03'){
...
module.exports = { handleSubscriptionPC03 }
