...
const { generarTags, generarTagsTA, generarTagsTCP, arregloTagsPC08Antenas } = require('../../../helpers/california/arregloTagsAireacion');
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PC08Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(5)] = ex.Value;     
                    });
                    dataCache.PC08Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPC08 = (ws, message) => {
    if (message.toString() === 'subscribePC08'){
    ...
    } else if (message.toString() === 'unsubscribePC08'){
...
module.exports = { handleSubscriptionPC08 }
