...
const { generarTags, generarTagsTA, arregloTagsPC16Antenas } = require('../../../helpers/california/arregloTagsAireacion');
let interval = null; 
let dataCache = {
    PC16: [],
    PC16TA: [],
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PC16Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(5)] = ex.Value;     
                    });
                    dataCache.PC16Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPC16 = (ws, message) => {
    if (message.toString() === 'subscribePC16'){
    ...
    } else if (message.toString() === 'unsubscribePC16'){
...
module.exports = { handleSubscriptionPC16 }
