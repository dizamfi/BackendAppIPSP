...
const { generarTags, generarTagsTA, generarTagsTCP, arregloTagsPC10Antenas } = require('../../../helpers/california/arregloTagsAireacion');
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.PC10Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(5)] = ex.Value;     
                    });
                    dataCache.PC10Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionPC10 = (ws, message) => {
    if (message.toString() === 'subscribePC10'){
    ...
    } else if (message.toString() === 'unsubscribePC10'){
...
module.exports = { handleSubscriptionPC10 }
