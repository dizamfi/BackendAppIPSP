...
const { arregloTagsCabala1, arregloTagsCabala1General, arregloTagsCabala1TCP, arregloTagsCabala1Antenas } = require('../../helpers/chanduy/arregloTags');
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.Cabala1Gral = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(5)] = ex.Value;     
                    });
                    dataCache.Cabala1Gral.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.Cabala1TCP = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(5)] = ex.Value;     
                    });
                    dataCache.Cabala1TCP.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.Cabala1Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(9)] = ex.Value;     
                    });
                    dataCache.Cabala1Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionCB1 = (ws, message) => {
    if (message.toString() === 'subscribeCB1'){
    ...
    } else if (message.toString() === 'unsubscribeCB1'){
...
module.exports = { handleSubscriptionCB1 }
