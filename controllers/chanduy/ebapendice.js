...
const { arregloTagsApendice, arregloTagsApendiceTCP, arregloTagsApendiceAntenas, arregloTagsApendiceGeneral } = require('../../helpers/chanduy/arregloTags');
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.ApendiceGral = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(4)] = ex.Value;     
                    });
                    dataCache.ApendiceGral.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.ApendiceTCP = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(4)] = ex.Value;     
                    });
                    dataCache.ApendiceTCP.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.ApendiceAntenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(8)] = ex.Value;     
                    });
                    dataCache.ApendiceAntenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionAPN = (ws, message) => {
    if (message.toString() === 'subscribeAPN'){
    ...
    } else if (message.toString() === 'unsubscribeAPN'){
...
module.exports = { handleSubscriptionAPN }
