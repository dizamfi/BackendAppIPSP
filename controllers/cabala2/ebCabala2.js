...
const { arregloTagsCabala2, arregloTagsCabala2General, arregloTagsCabala2TCP, arregloTagsCabala2Antenas } = require('../../helpers/chanduy/arregloTags');
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.Cabala2Gral = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(5)] = ex.Value;     
                    });
                    dataCache.Cabala2Gral.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.Cabala2TCP = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(5)] = ex.Value;     
                    });
                    dataCache.Cabala2TCP.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
    ...
    try {
        let client = net.connect('\\\\.\\pipe\\HmiRuntime', () => {
            ...
            rl.on('line', (line) => {
                ...
                if (obj.Message === 'NotifyReadTag') {
                    const arreglo = obj.Params.Tags;
                    dataCache.Cabala2Antenas = [];
                    arreglo.forEach((ex, i) => {
                        obje[ex.Name.slice(9)] = ex.Value;     
                    });
                    dataCache.Cabala2Antenas.push(obje)
                    obje = {}
                    completedFuntions++
                    checkAllFuntionsComplete()
                }
...
const handleSubscriptionCB2 = (ws, message) => {
    if (message.toString() === 'subscribeCB2'){
    ...
    } else if (message.toString() === 'unsubscribeCB2'){
...
module.exports = { handleSubscriptionCB2 }
